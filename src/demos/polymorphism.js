...
class Warrior extends Cell {
	...
	constructor(quadrille, p, location) {
		super(quadrille, p);
...
class Projectile {
	...
	constructor(p, quadrille, emoji, origin, target) {
		this.p = p;
		this.quadrille = quadrille;
...
function initialState(rows, cols, p) {
...
function mouseClicked(p) {
	const currentCoords = [quadrille.mouseRow, quadrille.mouseCol];
	...
	if (state.phase === Phase.Waiting && currentCell instanceof Protagonist) {
		...
		highlightQuadrille.visit(
			({ value: cell }) => cell.highlight(),
			({ row, col }) => currentCell.canReach([row, col]),
		);
		return;
	}
	...
	if (state.phase === Phase.Selected) {
		...
		highlightQuadrille.visit(({ value: cell }) => {
